...
from pipeline.embeddings import build_embeddings as emb_module
...
def _load_data() -> tuple[dict, dict, pd.DataFrame]:
    ...
    cust_path = DATA_DIR / "customer_data_200.csv"
    ...
    customers = pd.read_csv(cust_path, sep=";", dtype={"GUEST_ID": str, "HOTEL_ID": str})
    customers["GUEST_ID"] = customers["GUEST_ID"].str.strip().str.strip('"')
    customers["HOTEL_ID"] = customers["HOTEL_ID"].str.strip().str.strip('"')
    customers["CHECKIN_DATE"] = pd.to_datetime(customers["CHECKIN_DATE"])
    customers["CHECKOUT_DATE"] = pd.to_datetime(customers["CHECKOUT_DATE"])

    return embeddings, segments, customers
