...
OUTPUT_DIR = AUTONOMOUS_DIR / "output"
EMAILS_DIR = OUTPUT_DIR / "emails"
GENERIC_DIR = OUTPUT_DIR / "generic_campaigns"
ORACLE_FILE = OUTPUT_DIR / "oracle_context.json"
STATE_FILE = Path(os.environ.get("AUTONOMOUS_STATE_FILE", AUTONOMOUS_DIR / "state.json"))
...
def _resolve_credentials_path() -> Path | None:
    ...
    (p. ej. ``other-files/whatever-impact-….json``) para que el repositorio
    ...
    el proceso (``python autonomous/run.py``, ``python frontend/marketing/server.py``,
    tests, etc.).
    """
