...
que ``autonomous/state.json`` se escribe con semántica *last-writer-wins*.
...
def _warm_caches(force_mock: bool) -> None:
    ...
    if not force_mock:
        try:
            ...
            pass
    try:
        from pipeline.marketing import chat_engine

        chat_engine._get_dashboard()
    except Exception:
        pass
    ...
    try:
        ...
        for idx in range(campaigns_per_tick):
            ...
            try:
                ...
                proposal = chat_engine.generate_single_campaign_proposal(
                    idx,
                    previous_names,
                    force_mock=force_mock,
                )
