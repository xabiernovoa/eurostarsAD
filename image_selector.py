...
def _load_image_metadata(hotel_id: str) -> list[dict]:
    ...
    meta_path = os.path.join(IMAGES_DIR, str(hotel_id), "metadata.json")
...
def select_images(
    ...
    for img in metadata:
        ...
        img_path = os.path.join(IMAGES_DIR, str(hotel_id), img["filename"])
        scored.append({
            "filename": img["filename"],
            "path": img_path if os.path.exists(img_path) else PLACEHOLDER_IMAGE,
            "score": score,
            "tags": img.get("tags", []),
            "is_placeholder": not os.path.exists(img_path),
        })
